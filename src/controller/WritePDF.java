    ...
    public void writePhieuXuat(String mapn) {
        ...
        try {
            ...
            for (ChiTietPhieu ctpn : ChiTietPhieuXuatDAO.getInstance().selectAll(mapn)) {
                ...
                double giaXuat = mt.getGia() * 1.1; // ✅ giá xuất
                int soLuong = ctpn.getSoLuong();
                double thanhTien = giaXuat * soLuong;

                pdfTable.addCell(new PdfPCell(new Phrase(ctpn.getMaMay(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(mt.getTenMay(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(giaXuat) + "đ", fontData)));        // ✅ đơn giá xuất
                pdfTable.addCell(new PdfPCell(new Phrase(String.valueOf(soLuong), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(thanhTien) + "đ", fontData)));      // ✅ tổng tiền theo giá xuất
            }
