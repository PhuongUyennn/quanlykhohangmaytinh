...
public class LaptopDAO implements DAOInterface<Laptop> {
    ...
    public int insert(Laptop t) {
        ...
        try {
            ...
            String sql = "INSERT INTO MayTinh (maMay, tenMay, soLuong, tenCpu, ram, cardManHinh, gia, dungLuongPin, kichThuocMan, xuatXu, loaiMay, rom, trangThai, gia_xuat) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(Laptop t) {
        ...
        try {
            ...
            String sql = "UPDATE MayTinh SET maMay=?, tenMay=?, soLuong=?, tenCpu=?, ram=?, cardManHinh=?, gia=?, dungLuongPin=?, kichThuocMan=?, xuatXu=?, loaiMay = ?, rom = ?, trangThai = ?, gia_xuat = ? WHERE maMay=? ";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaMay());
            pst.setString(2, t.getTenMay());
            pst.setInt(3, t.getSoLuong());
